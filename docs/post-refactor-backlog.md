...
| 27 | 2026-08-23 | **OPCIONAL — centralizar la derivación `role + host_color -> local_color`** | La misma regla aparece en el reducer común, la configuración Spectrum y la aplicación de partidas guardadas. Experimentar con una representación compartida solo si conserva configuración, saves, wire y comportamiento. Aceptar únicamente con menos duplicación y cero crecimiento enlazado en resident/BSS/overlays de Classic, Next-TAP y NEX; revertir si añade una capa, llamadas o bytes sin eliminar una fuente real de estado. | 🔵 experimento medido |
| 28 | 2026-08-23 | **OPCIONAL — extraer el control común de los loaders Classic y Next** | `overlay_loader.asm` y `overlay_loader_next.asm` comparten caché, selección/validación de atlas y dispatch, pero difieren en esxDOS frente a MMU/banking. Probar primero con include/macros privados y comparar listados enlazados. Aceptar solo si no crece ningún target y conserva ABI, slot de 2048 bytes, ownership de DI/EI, restauración IX/IY y páginas MMU; rollback ante cualquier divergencia o ahorro meramente textual. | 🔵 experimento ASM |
| 29 | 2026-08-23 | **OPCIONAL — sustituir el lookup lineal de piezas de RESTORE** | Medir alternativas para `restore_find_piece()`/`save_piece_nibble()` sin tocar `NETCHESSZX_SAVE_PIECE_TABLE`, el nibble reservado, CRC, Base64 ni errores públicos. Aceptar solo con todos los vectores golden/negativos verdes y reducción enlazada real de RESTORE sin coste resident; revertir si RESTORE queda igual, crece o la representación resulta menos clara. | 🔵 experimento medido |
