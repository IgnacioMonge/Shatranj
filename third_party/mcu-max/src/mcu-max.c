...
size_t mcumax_state_size(void)
{
    return sizeof(mcumax);
}

#ifndef NETCHESSZX_FIXED_LOW_RAM
void mcumax_get_state(void *out)
{
    if (out != NULL)
        memcpy(out, &mcumax, sizeof(mcumax));
}

void mcumax_set_state(const void *in)
{
    if (in != NULL)
        memcpy(&mcumax, in, sizeof(mcumax));
}
#endif
