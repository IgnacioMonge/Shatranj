...
bool DesktopSessionAdapter::init(uint8_t transport,
                                 uint8_t role,
                                 uint8_t hostColor,
                                 uint16_t sessionId)
{
...
bool DesktopSessionAdapter::initDirect(uint8_t role, uint8_t hostColor)
{
    return init(SESSION_TRANSPORT_DIRECT, role, hostColor, 0u);
}

bool DesktopSessionAdapter::initMqtt(uint8_t role,
                                     uint8_t hostColor,
                                     uint16_t sessionId)
{
    return init(SESSION_TRANSPORT_MQTT, role, hostColor, sessionId);
}

void DesktopSessionAdapter::enqueue(OwnedEvent event, bool first)
{
    /* Correlated completions, and MQTT link loss, must run before external
       events already queued behind the action batch which created them. */
    if (first) {
...
void DesktopSessionAdapter::enqueueLinkUp(uint8_t linkId)
{
    OwnedEvent owned;
...
void DesktopSessionAdapter::enqueueLinkDown(uint8_t linkId)
{
    OwnedEvent owned;
    ...
    owned.event.data.link.link_id = linkId;
    enqueue(std::move(owned),
            state_.config.transport == SESSION_TRANSPORT_MQTT);
}

bool DesktopSessionAdapter::enqueueRx(uint8_t linkId,
                                      uint8_t route,
                                      uint8_t flags,
                                      const QByteArray &payload)
{
    ...
    owned.event.data.rx.flags = flags;
...
void DesktopSessionAdapter::enqueueDirectRx(uint8_t linkId,
                                            const QByteArray &payload)
{
    (void)enqueueRx(linkId, SESSION_ROUTE_DEFAULT, SESSION_RX_LIVE, payload);
}
...
bool DesktopSessionAdapter::enqueueMqttRx(uint8_t linkId,
                                          ...
                                          const QByteArray &payload)
{
    ...
    if (payload.size() > SESSION_PAYLOAD_MAX ||
        !mqttRouteForTopic(topic, &route)) {
        return false;
    }
    return enqueueRx(linkId, route,
                     retained ? SESSION_RX_RETAINED : SESSION_RX_LIVE,
                     payload);
}

QByteArray DesktopSessionAdapter::mqttTopicSuffixForRoute(uint8_t route) const
{
...
void DesktopSessionAdapter::enqueueLocalRequest(uint8_t request,
                                                uint16_t value,
                                                const QByteArray &payload,
                                                uint8_t phase)
{
...
void DesktopSessionAdapter::enqueueUserDecision(uint8_t requestId,
                                                uint8_t decision)
{
    OwnedEvent owned;
...
void DesktopSessionAdapter::enqueueTxResult(uint8_t txId, uint8_t result)
{
    OwnedEvent owned;
...
void DesktopSessionAdapter::enqueueTimeout(uint8_t timerId)
{
    OwnedEvent owned;
...
void DesktopSessionAdapter::enqueueGameResult(uint8_t deliveryId,
                                              uint16_t value,
                                              uint8_t result,
...
bool DesktopSessionAdapter::takeNextBatch(DesktopActionBatch *batch)
{
    OwnedEvent owned;
    uint8_t count;
    ...
    for (uint8_t i = 0u; i < count; ++i) {
        DesktopOwnedAction &out = batch->actions[i];
